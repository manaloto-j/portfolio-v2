...
// ─── Trail Decay ──────────────────────────────────────────────────────────────
export const DECAY_ALPHA = 0.9;
export const DECAY_SIZE = 0.6;

/** Pre-computed per-segment alpha & size decay tables (avoids pow() per frame) */
export const ALPHA_DECAY = Float32Array.from({ length: MAX_TAIL_LEN }, (_, i) =>
  Math.pow(DECAY_ALPHA, i),
);
export const SIZE_DECAY = Float32Array.from({ length: MAX_TAIL_LEN }, (_, i) =>
